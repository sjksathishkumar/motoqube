...
function difference($val1)
{
	...
	if($months!='' && $days!=''){ $time = $months." Month & ".$days." ".$day; } elseif($months!=''){ $time = $months." Month"; }elseif($days!=''){ $time = $days." ".$day; }elseif($hours!='') { $time = $hours." Hour"; } else{ $time = $minuts." Minutes"; }	//echo $time;	
	return $time;
}
function timeCal($val){
    ...
    if ($date == date('Y-m-d')) {
        $now_time = date('H:i:s');
        $pos_time = date('H:i:s', strtotime($val));
        $diff = round(abs(strtotime($now_time) - strtotime($pos_time)) / 60, 0);
        $post_time = $diff;
        $post_date = $post_time . " mins ago";
        if ($post_time > 60) {
            $post_time = round($post_time / 60, 0);
            $post_date = $post_time . " hrs ago";
            if ($post_time > 23) {
                $post_date = date('M jS Y', strtotime($val));
                $post_time = date('h:i a', strtotime($val));
                $post_date = $post_date .' at '.$post_time;
            }
        }
    } else {
        $post_date = date('M jS Y', strtotime($val));
...
function timeCaldate($val){
    ...
    if ($date == date('Y-m-d')) {
        ...
        if ($post_time > 60) {
            ...
            $post_date = $post_time . "hrs ago";
            if ($post_time > 23) {
                $post_time = date('d-m-Y', strtotime($val));
                $post_date = $post_time;
            }
        }
    } else {
        $post_date = date('Y-m-d', strtotime($val));
...
//	echo $aryOpts." ".$pgCnt." ".$curPg;
	...
	{
		$aryOpts['pg']=$i;
		if($pgCnt==$i && $curPg>=$pgCnt) $class.=' bordernone';
...
function msg($msg)
{
 ...
 return '<div class="alert alert-'.$type.'">
	...
	if($msg[$type]!='')
	{
	 return '<div class="'.$type.'"><p class="closestatus"><font color="#F7B4C9">X</font> '.$content.'</p></div>';
	}
}
...
function mail_template_user($to,$result,$subject,$out_going,$attach_temp,$attachment)
{ 
	...
	$mail = new PHPMailer();
	$mail->IsSMTP();
	$mail->Host 	=  $out_going[0];
	$mail->SMTPDebug= 1;
	$mail->SMTPAuth = $out_going[4];
	$mail->Port 	= $out_going[1];
        $mail->IsHTML(); 
	$mail->Username	= $out_going[2];
	$mail->Password = $out_going[3];
	$mail->SMTPSecure = $out_going[5];
        $mail->SetFrom($out_going[2],"Admin");
...
//send to multiple address
	...
	foreach($to as $to_add){
        ...
        $mail->AddAttachment($attach_temp,$attachment);      // attachment
	if(!$mail->Send()) {
	...
	//$headers  = 'MIME-Version: 1.0' . "\r\n";
	//$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
	//$headers .= 'From: radhakrishnan@foyertech.com Jhumuka' . "\r\n";
...
{ 
    ...
    $mail_settings = $db->getRow("select * from smtp_settings where id = '1'");
	...
	//$mail->SMTPSecure = "tls";
        $mail->SMTPSecure = "ssl";
	$mail->SetFrom($mail_settings['username'],"Admin");
	$mail->Subject= $subject;
	$mail->AltBody= "To view the message, please use an HTML compatible email viewer!";
	$mail->MsgHTML($result);
	$mail->AddAddress($to);
...
function getPageDesc($linkname='',$type='')
{
	if($linkname!="")
	{
		$data = mysql_query("select short_description,pbody,id from cms where id='".$linkname."'");
		$iData =  mysql_fetch_array($data);
...
function filterwords($text, $badwords){
 $filterWords = $badwords;
 $filterCount = sizeof($filterWords);
 for($i=0; $i<$filterCount; $i++){
  $text = preg_replace('/\b'.$filterWords[$i].'\b/ie',"str_repeat('<span style='color:red;'>".$filterWords[$i]."</span>',strlen('$0'))",$text);
 }
 return $text;
}
function censor_words_chech($text, $badwords){
    $url_string = explode(' ', $text);
    foreach($url_string as $key => $val){
    if (in_array($val, $badwords)) { // Yoshi version       
        return true;
    }
}
return false;
...
function highlight($text, $words) {
    ...
    if(!$m)
        return $text;
    $re = '~\\b(' . implode('|', $m[0]) . ')\\b~';
    return preg_replace($re, '<span class="red_color">$0</span>', $text);
}
...
function upload_new_single_file($field_name, $directory_path, $add_extensions='', $change_file_name='')
{
	if( !empty($field_name) && !empty($directory_path) )
  	{
		...
		if( $_FILES[$field_name]['name'] != '' )
		{
			...
			{
			  	$file_extensions = array('jpeg', 'jpg', 'gif', 'png');
			}
			else
			{
			  	$file_extensions = array('jpeg', 'jpg', 'gif', 'png');
...
function upload_new_multiple_file($field_name, $directory_path, $counts, $add_extensions='', $change_file_name='')
{
	if( !empty($field_name) && !empty($directory_path) )
  	{
  		$field_name = trim($field_name);
		...
		if( $_FILES[$field_name]['name'][$counts] != '' )
		{
			$file_extensions = '';
			...
			$file_name = $_FILES[$field_name]['name'][$counts];
			...
			if( in_array($extension, $file_extensions) )
			{
				...
				{
					$new_file_name = md5($_FILES[$field_name]['tmp_name'][$counts]).".".$extension;
				}

				if( move_uploaded_file($_FILES[$field_name]['tmp_name'][$counts], $directory_path.$new_file_name) )
				{
					$file_upload_status = 'uploaded';
				}
				else
...
function uploaded_new_single_file_result($file_uploaded_result, $result_field_name='')
{
	if( !empty($file_uploaded_result) )
	{
		if( !empty($result_field_name) )
		{
			$result_field_name = ucfirst($result_field_name);
		}
		else
		{
			$result_field_name = 'File';
		}

		if( $file_uploaded_result['file_upload_status'] == 'uploaded' )
		{
			$_SESSION['success'] = "$result_field_name Successfully Uploaded";
		}
		else if( $file_uploaded_result['file_upload_status'] == 'not_uploaded' )
		{
			$_SESSION['error'] = "$result_field_name Not Uploaded";
		}
		else if( $file_uploaded_result['file_upload_status'] == 'invalid_format' )
		{
			$_SESSION['error'] = "Invalid File Format";
		}
		else if( $file_uploaded_result['file_upload_status'] == 'not_exist' )
		{
			$_SESSION['error'] = "File Not Exist";
		}
	}
}

function unlink_uploaded_old_single_file($delete_file_path)
{
	if( !empty($delete_file_path) )
	{
...
function get_and_unlink_uploaded_old_single_file($file_field_name, $table_name, $field_name, $field_value)
{
	if( !empty($file_field_name) && !empty($table_name) && !empty($field_name) && !empty($field_value) )
  	{
		global $db;

		$delete_file_path = $db->getVal("select $file_field_name from $table_name where $field_name = '".$field_value."'");
...
function database_tables_actions_result($action_result, $success_message, $success_redirect_location, $error_message, $error_redirect_location='')
{
	if( !is_null($action_result) && !empty($success_message) && !empty($success_redirect_location) && !empty($error_message) )
  	{
  		$redirect_location = $success_redirect_location;
		...
		else
		{
			...
			if( empty($error_redirect_location) )
	  		{
	  			redirect($redirect_location);
	  		}
	  		else
	  		{
	  			redirect($error_redirect_location);
	  		}
		}
	}
}

function unset_session_success_and_session_error()
{
	$session_status = array();
	
	if( isset($_SESSION['success']) && ($_SESSION['success'] != "") ) 
	{
		$session_status['success'] = $_SESSION['success'];
		unset($_SESSION['success']);
	}

	if( isset($_SESSION['error']) && ($_SESSION['error'] != "") ) 
	{
		$session_status['error'] = $_SESSION['error'];
