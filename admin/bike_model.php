...
$result = $db->getRow("Select make from sp_bike_make where make_id='$make_id'");
		...
		<div class="container-fluid">
	        <div class="row panel panel-default page-wrapper-panel">
		        <div class="gradient-card-header">
		            <h2 class="white-text mx-3"> Bike Models</h2>
		        </div>
                <div ><button class="btn btn-info" onclick="window.location.href='bike_make.php'"><i class="fa fa-angle-left"></i> Go Back</button> <div class="breadcrumb"><span><a href="bike_make.php"><?= $make_name ?></a> </span> / <span>Bike Models </span>
                </div>
		              ...
		              <div class="panel-body">
		                <form method="post" id="content_add"   enctype="multipart/form-data">
		                  ...
		                  <div class="col-md-4 top15">
		                    ...
		                    <input  type="file" name="model_image" style="margin-top: 10px;" required="" />
		                  </div>
...
<script type="text/javascript">
  ...
  $('#bike_model_table').dataTable({
    ...
    "serverSide": true,
    "bAutoWidth": false,
    "ajax": "ajax_table_load.php?make=<?=$make_id?>&action=bike_model_table"
  });
});
$(document).on('click', '.btnTableEdit', function( event ) {
    ...
    $.ajax({
        url: 'ajax_load.php',
        type: 'POST',
        data: 'action=edit_bike_model&id='+value,
        dataType: 'html'
    })
    .done(function(data){
...
$(document).ready(function(e){
    $("#content_add").on('submit', function(e){
        ...
        formData.append('make', '<?=$make_id?>');    
    		formData.append('action', 'insert_bike_model');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
    ...
    $("#edit_custom_form").on('submit', function(e){
        e.preventDefault();
    		var formData = new FormData(this);
    		formData.append('action', 'update_bike_model');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
