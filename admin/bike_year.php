...
$result = $db->getRow("Select a.make,a.make_id, b.model from sp_bike_make a left join sp_bike_model b on a.make_id = b.make_id where model_id='$model_id' Limit 1 ");
		...
		<div class="container-fluid">
	        <div class="row panel panel-default page-wrapper-panel">
		        <div class="gradient-card-header">
		            <h2 class="white-text mx-3"> Bike Years</h2>
		        </div>
            <div ><button class="btn btn-info" onclick="window.location.href='bike_model.php?make=<?=$make_id?>'"><i class="fa fa-angle-left"></i> Go Back</button> <div class="breadcrumb"><span><a href="bike_make.php"><?= $make_name ?></a></span> / <span><a href="bike_model.php?make=<?=$make_id?>"><?= $model_name ?></a> </span> / <span>Bike Years </span>
            </div>
...
<script type="text/javascript">
  ...
  $('#bike_model_table').dataTable({
    ...
    "serverSide": true,
    "bAutoWidth": false,
    "ajax": "ajax_table_load.php?model=<?=$model_id?>&action=bike_year_table"
  });
});
$(document).on('click', '.btnTableEdit', function( event ) {
    ...
    $.ajax({
        url: 'ajax_load.php',
        type: 'POST',
        data: 'action=edit_bike_year&id='+value,
        dataType: 'html'
    })
    .done(function(data){
...
$(document).ready(function(e){
    $("#content_add").on('submit', function(e){
        ...
        formData.append('model', '<?=$model_id?>');                       
    		formData.append('action', 'insert_bike_year');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
