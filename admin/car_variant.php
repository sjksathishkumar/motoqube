...
$result = $db->getRow("Select b.make,b.make_id, c.model,c.model_id, a.year, a.year_id from sp_car_year a left join sp_car_make b on a.make_id = b.make_id left join sp_car_model c on a.model_id = c.model_id  where a.year_id='$year_id' Limit 1 ");
if(count($result)>0){$make_name = $result["make"];$make_id = $result["make_id"];$model_name = $result["model"];$model_id = $result["model_id"];$year_name = $result["year"];/*$variant_name = $result["variant"];$variant_id = $result["variant_id"];*/}else{header('Location: index.php');}
?>
</head>
<body>
<!--------------------->
		...
		<div class="container-fluid">
	        <div class="row panel panel-default page-wrapper-panel">
		        <div class="gradient-card-header">
		            <h2 class="white-text mx-3"> Car Variants</h2>
		        </div>
            <div ><button class="btn btn-info" onclick="window.location.href='car_year.php?model=<?=$model_id?>'"><i class="fa fa-angle-left"></i> Go Back</button> <div class="breadcrumb"><span><a href="car_make.php"><?= $make_name ?></a></span> / <span><a href="car_model.php?make=<?=$make_id?>"><?= $model_name ?></a> </span> / <span><a href="car_year.php?model=<?=$model_id?>"><?= $year_name ?></a> </span> / <span>Car Variants </span>
            </div>
...
<script type="text/javascript">
  ...
  $('#car_model_table').dataTable({
    ...
    "serverSide": true,
    "bAutoWidth": false,
    "ajax": "ajax_table_load.php?year=<?=$year_id?>&action=car_variant_table"
  });
});
$(document).on('click', '.btnTableEdit', function( event ) {
    ...
    $.ajax({
        url: 'ajax_load.php',
        type: 'POST',
        data: 'action=edit_car_variant&id='+value,
        dataType: 'html'
    })
    .done(function(data){
...
$(document).ready(function(e){
    $("#content_add").on('submit', function(e){
        ...
        formData.append('year', '<?=$year_id?>');                                                     
    		formData.append('action', 'insert_car_variant');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
