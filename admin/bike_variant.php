<?php if(isset($_GET["year"])){$year_id = $_GET["year"];}else{header('Location: index.php');} 
include 'inc-meta.php';
$result = $db->getRow("Select b.make,b.make_id, c.model,c.model_id, a.year, a.year_id from sp_bike_year a left join sp_bike_make b on a.make_id = b.make_id left join sp_bike_model c on a.model_id = c.model_id  where a.year_id='$year_id' Limit 1 ");
// echo $db->getLastQuery();exit;
if(count($result)>0){$make_name = $result["make"];$make_id = $result["make_id"];$model_name = $result["model"];$model_id = $result["model_id"];$year_name = $result["year"];}else{header('Location: index.php');}
?>
</head>
<body>
<!--------------------->
		...
		<div class="container-fluid">
	        <div class="row panel panel-default page-wrapper-panel">
		        <div class="gradient-card-header">
		            <h2 class="white-text mx-3"> Bike Variants</h2>
		        </div>
            <div ><button class="btn btn-info" onclick="window.location.href='bike_year.php?model=<?=$model_id?>'"><i class="fa fa-angle-left"></i> Go Back</button> <div class="breadcrumb"><span><a href="bike_make.php"><?= $make_name ?></a></span> / <span><a href="bike_model.php?make=<?=$make_id?>"><?= $model_name ?></a> </span> / <span><a href="bike_year.php?model=<?=$model_id?>"><?= $year_name ?></a> </span> / <span>Bike Variants </span>
            </div>
		              ...
		              <div class="panel-body">
		                <form method="post" id="content_add"   enctype="multipart/form-data">
		                  <div class="col-md-4 top15">
		                    <span class="badge-label info-color p-2">Variants<span class="redstar">*</span></span>
		                    <input type="text" id="variant" name="variant" class="form-control" placeholder="Variants" value="" required="">
		                  </div>
		        ...
		        <div class="row">
	                <div class="panel-default">
	                  <div class="panel-body">
	                    <table id="bike_model_table" class="table table-striped table-hover" width="100%">
	                      <thead>
	                        <tr>
	                          <th>ID</th>   
	                          <th>Variant</th>             
...
<script type="text/javascript">
  ...
  $('#bike_model_table').dataTable({
    ...
    "serverSide": true,
    "bAutoWidth": false,
    "ajax": "ajax_table_load.php?year=<?=$year_id?>&action=bike_variant_table"
  });
});
$(document).on('click', '.btnTableEdit', function( event ) {
    ...
    $.ajax({
        url: 'ajax_load.php',
        type: 'POST',
        data: 'action=edit_bike_variant&id='+value,
        dataType: 'html'
    })
    .done(function(data){
...
$(document).ready(function(e){
    $("#content_add").on('submit', function(e){
        ...
        formData.append('year', '<?=$year_id?>');                                                     
    		formData.append('action', 'insert_bike_variant');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
    ...
    $("#edit_custom_form").on('submit', function(e){
        e.preventDefault();
    		var formData = new FormData(this);
    		formData.append('action', 'update_bike_year');
        $.ajax({
            type: 'POST',
            url: 'ajax.php',
            data: formData,
